...
class ServiceOracleDoctores:
    ...
    def getAllDoctores(self):
        ...
        for row in cursor:
            ...
            doc.salario = row[4]
            doc.hospital = row[0]
            datos.append(doc) # cada doctor que encontramos, lo añadimos a la lista
        cursor.close()
        return datos
    
    def insertarDoctor(self, iddoctor, ape, espe, hosp, sala):
        sql = "insert into DOCTOR values (:hosp, :id, :ape, :espe, :sal)"
        cursor = self.connection.cursor()
        cursor.execute(sql, (hosp, iddoctor, ape, espe, sala )) # en el orden de la tabla
    ...
    def modificarDoctor(self, iddoctor, apellido, especialidad, salario, hospital):
        sql = """
            update DOCTOR set APELLIDO=:p1, ESPECIALIDAD=:p2, SALARIO=:p3,
            HOSPITAL_COD=:p4
            where DOCTOR_NO=:p5
        """
        cursor = self.connection.cursor()
